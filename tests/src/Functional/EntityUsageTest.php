...
class EntityUsageTest extends BrowserTestBase {
  ...
  protected function setUp() {
    ...
    $this->drupalPostForm('admin/config/content/entity_usage', $edit, $this->t('Save configuration'));
  ...
  public function testAdminTitle() {
    ...
    $this->drupalGet('/entity_usage/node/1');
    $this->assertSession()->statusCodeEquals(200);
    $this->assertSession()->responseContains('node 2');
    $this->assertSession()->responseContains('node 3');

    $this->drupalGet('/entity_usage/node/2');
    $this->assertSession()->statusCodeEquals(200);
    $this->assertSession()->responseContains('node 3');

    $this->drupalGet('entity_usage/node/3');
    $this->assertSession()->statusCodeEquals(200);
